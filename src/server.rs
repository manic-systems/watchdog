...
pub async fn run(config: Config, build_info: BuildInfo) -> anyhow::Result<()> {
    ...
    let state = AppState::new(config, build_info).context("failed to initialize application")?;
    ...
    let uniques_task = spawn_unique_gauge_updater(state.clone(), shutdown.child_token());
...
fn spawn_unique_gauge_updater(state: AppState, shutdown: CancellationToken) -> JoinHandle<()> {
