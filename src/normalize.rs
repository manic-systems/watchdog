...
pub fn extract_referrer_domain(referrer: &str, site_domain: &str) -> Option<String> {
...
pub fn extract_referrer_url(referrer: &str, site_domain: &str) -> Option<String> {
    let host = normalized_referrer_host(referrer, site_domain)?;
...
fn normalized_referrer_host(referrer: &str, site_domain: &str) -> Option<ReferrerHost> {
...
mod tests {
    ...
    fn extracts_effective_referrer_domain() {
        assert_eq!(
            extract_referrer_domain("https://news.ycombinator.com/item?id=1", "example.com"),
